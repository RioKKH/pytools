...
def cmp_hist2(df1, df2) -> None:
    fig, ax0 = plt.subplots(nrows=1, ncols=1)
    bins = range(df1['num'].min(), df1['num'].max() +2)
    df1['num'].plot.hist(bins=bins, align='left', ax=ax0, alpha=0.5, label='original')
    df2['num'].plot.hist(bins=bins, align='left', ax=ax0, alpha=0.5, label='myModel')
...
def heatmap(df1, df2) -> None:
    fig, [ax0, ax1] = plt.subplots(nrows=1, ncols=2, figsize=(9, 6))
    reg = df1.pivot_table(columns="wo", index="w1", values="num")[::-1]
    mym = df2.pivot_table(columns="wo", index="w1", values="num")[::-1]
